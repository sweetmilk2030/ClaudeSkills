...
### Lỗi 2 — Commentary bị cắt đôi bởi layout 2 cột (QUAN TRỌNG)
...
Đọc trang gốc OCR để xem `[đoạn B]` nằm ở đâu:

```bash
# Đọc 2 trang đầu của chapter
...
Nếu `[đoạn B]` xuất hiện **trước** `1—4.` → dùng `cp()` riêng.
...
C.push(com('3, 4.', 'Ahaziah sends to this deity as to the probability...'));         // trang 2
C.push(com('5—8.', 'The messengers did not recognise Elijah...'));                    // trang 2
...
## Helper functions (view_skill.js)
...
// ⚠️  Xem LỖI #2 trước khi quyết định dùng cp() hay gộp vào com() trước đó
...
8. **`cp()` CHỈ sau khi xác nhận OCR** — không dùng cp() cho đoạn có thể là phần bị cắt của com() trước (xem LỖI #2)
...
CHAPTER I
  ...
  com("1—4.", "These two Books...")     ← ⚠️ gộp đủ cả phần bị layout cắt xen (xem LỖI #2)
  com("5.", "Another note...")
...
| II Kings (Vol.2) | OCR txt | 25 | 1082 | `Cassells_1860_II_KINGS_full.docx` |
...
| `cp()` sai chỗ — đoạn thực ra thuộc com() trước | Layout 2 cột cắt entry commentary | Xem LỖI #2, đọc OCR raw 2 trang liên tiếp để xác nhận |
