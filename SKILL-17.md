...
11. **Chạy PROTOCOL 5 bước cho TỪNG chương, không có ngoại lệ** — kể cả khi đã đọc toàn bộ OCR trước đó: grep xác định trang → cat đọc RAW → grep đếm entries → in danh sách com() → sed kiểm tra câu cuối. Đọc OCR gộp một lần không thay thế được Protocol này.
