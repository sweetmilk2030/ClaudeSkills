...
>
> **Đây là bước không thể bỏ qua.** Mọi lỗi đã gặp (Lỗi 4, Lỗi 5) đều xảy ra vì AI bỏ qua bước này và viết code từ trí nhớ hoặc đọc thiếu. Checklist thụ động không đủ — phải chạy lệnh bash thực sự và in kết quả ra trước khi viết bất kỳ dòng code nào.
...
**Bước B — Đọc RAW toàn bộ trang liên quan (KHÔNG được bỏ trang nào):**

```bash
# Ví dụ nếu Ch.III commentary nằm trên trang 3 và 4:
cat /tmp/amos/3.txt
cat /tmp/amos/4.txt
# Nếu chưa thấy "CHAPTER IV." → đọc tiếp trang 5, 6...
```

> Không được đọc một trang rồi bắt đầu code. Phải đọc HẾT tất cả trang liên quan trước.
...
**Bước E — Sau khi viết TỪNG com(), chạy lệnh kiểm tra câu cuối:**

```bash
# Lấy vài dòng OCR ngay sau vị trí đã dùng cho com() vừa viết
...
Bước A: grep xác định trang bắt đầu / kết thúc
   ↓
Bước B: cat đọc RAW toàn bộ trang liên quan
   ↓
Bước C: grep đếm số footnote entries
   ↓
Bước D: in danh sách com() sẽ viết (ghi trang nguồn)
   ↓
Bước E: viết code từng com() → sau mỗi cái: sed kiểm tra câu cuối
   ↓
...
> ⚠️ **Việc đã đọc toàn bộ OCR trước đó KHÔNG miễn cho AI khỏi chạy Protocol này.** Mỗi chương vẫn phải chạy đủ 5 bước độc lập — vì chỉ có grep/cat/sed mới phát hiện được tràn trang và câu bị cụt, không phải trí nhớ từ lần đọc trước.
