...
## PROTOCOL BẮT BUỘC TRƯỚC KHI VIẾT COM() — KHÔNG CÓ NGOẠI LỆ

> **CẢNH BÁO CỨNG — ĐỌC TRƯỚC KHI LÀM BẤT CỨ ĐIỀU GÌ:**
>
> Lỗi phổ biến nhất và nguy hiểm nhất là: **đọc toàn bộ OCR bằng một lệnh duy nhất (`for i in ...; do cat; done`) rồi bắt đầu code ngay** — điều này khiến Protocol 5 bước KHÔNG BAO GIỜ được chạy, vì AI tưởng mình đã đọc đủ rồi.
>
> **QUY TẮC SẮT:** Đọc toàn bộ OCR để nắm nội dung là được, nhưng **TRƯỚC KHI VIẾT `com()` cho bất kỳ chương nào**, bắt buộc phải dừng lại và chạy đủ Protocol 5 bước riêng cho chương đó — kể cả khi đã đọc file đó rồi. Không có ngoại lệ, không có chương nào được bỏ qua.
>
> **Đây là bước không thể bỏ qua.** Mọi lỗi đã gặp (Lỗi 4, Lỗi 5, Lỗi 6) đều xảy ra vì AI bỏ qua bước này và viết code từ trí nhớ hoặc đọc thiếu. Checklist thụ động không đủ — phải chạy lệnh bash thực sự và in kết quả ra trước khi viết bất kỳ dòng code nào.

> **⛔ LỆNH CẤM TUYỆT ĐỐI — KHÔNG ĐƯỢC DÙNG TRƯỚC KHI CODE:**
>
> ```bash
> # CẤM — đọc gộp nhiều trang một lần trước khi bắt đầu viết com()
> for i in $(seq 1 10); do cat /tmp/book/$i.txt; done
> cat /tmp/book/*.txt
> ```
>
> Lý do: Khi đọc gộp, context window có toàn bộ text nhưng AI **không thể xác định chính xác ranh giới tràn trang** của từng entry. Cụ thể: đoạn nối tiếp của một `com()` có thể nằm ở trang sau, xen giữa verses của chương tiếp theo trong OCR tuyến tính — mắt thường không thể phát hiện, chỉ có `grep -n` và `sed -n` mới làm được. Đây chính xác là nguyên nhân gây ra Lỗi #6 (AMOS Ch.III, com '12.' thiếu đoạn "Damascus having been conquered...").

### Bước bắt buộc cho TỪNG chương, theo đúng thứ tự

**Bước A — Xác định commentary nằm trên những trang nào:**

```bash
# Tìm trang bắt đầu và trang kết thúc commentary chương N
grep -rl "CHAPTER III\." /tmp/amos/    # trang có "CHAPTER III." lần 2 (bottom)
grep -rl "CHAPTER IV\." /tmp/amos/    # trang có "CHAPTER IV." → Ch.III kết thúc trước đây
```

**Bước B — Xác định ranh giới tràn trang bằng grep -n, rồi đọc RAW:**

Đây là bước quan trọng nhất. Phải chạy **hai lệnh** theo thứ tự:

```bash
# B-1: Dùng grep -n để biết CHÍNH XÁC dòng nào bắt đầu commentary Ch.N
#      và dòng nào bắt đầu Ch.N+1 (= ranh giới kết thúc)
grep -n "CHAPTER III\." /tmp/amos/3.txt   # → dòng bắt đầu commentary
grep -n "CHAPTER IV\."  /tmp/amos/4.txt   # → dòng kết thúc (Ch.N+1)

# B-2: Đọc RAW từng trang liên quan — bắt đầu từ trang chứa commentary,
#      đọc TIẾP cho đến khi thấy "CHAPTER N+1." trong output
cat /tmp/amos/3.txt
cat /tmp/amos/4.txt   # ← ĐỌC CẢ TRANG NÀY dù tưởng Ch.III chỉ ở trang 3
# Nếu chưa thấy "CHAPTER IV." → đọc tiếp trang 5, 6...
```

> **Tại sao phải dùng `grep -n` trước?** Vì OCR tuyến tính trộn lẫn: đoạn cuối của `com('12.')` Ch.III có thể nằm ở trang 4, xen giữa verses Ch.IV. Nếu không dùng `grep -n` để định vị, AI đọc trang 4 nhưng không nhận ra đoạn đó thuộc Ch.III — vì context đã "trôi". `grep -n` cho số dòng cụ thể, buộc AI phải đọc đến đúng dòng đó.
>
> **Không được đọc một trang rồi bắt đầu code.** Phải đọc HẾT tất cả trang liên quan và thấy "CHAPTER N+1." trong output thì mới dừng.

**Bước C — Đếm và liệt kê tất cả footnote entries trong OCR:**

```bash
# In ra tất cả dòng bắt đầu bằng số footnote trong các trang liên quan
grep -hE "^[0-9]+([ ,\.—–-])" /tmp/amos/3.txt /tmp/amos/4.txt
```

**Bước D — In danh sách com() sẽ code, ghi rõ trang nguồn:**

AI phải in ra danh sách này trong response trước khi viết code:

```
CHAPTER III commentary — sẽ gồm:
  com('1.',   ...) ← trang 3
  com('2.',   ...) ← trang 3
  com('3—6.', ...) ← trang 3
  com('9.',   ...) ← trang 3
  com('12.',  ...) ← trang 3 + trang 4  ← tràn trang
  com('15.',  ...) ← trang 4
  Tổng: 6 entries
```

Số entries này phải khớp với kết quả đếm ở Bước C. Nếu không khớp → đọc lại.

**Bước E — Sau khi viết TỪNG com(), chạy hai lệnh kiểm tra:**

```bash
# E-1: Lấy vài dòng OCR ngay SAU vị trí đã dùng cho com() vừa viết
sed -n '128,145p' /tmp/amos/4.txt

# E-2: Kiểm tra xem dòng tiếp theo có phải là PHẦN NỐI TIẾP không
# Dấu hiệu nối tiếp: câu bắt đầu bằng chữ thường, không có số footnote mới,
# không có "CHAPTER", không phải cross-ref
```

> **Câu cuối của mỗi `com()` PHẢI kết thúc bằng dấu chấm `.`**
> Nếu chưa có dấu chấm → đọc tiếp OCR ngay bên dưới để lấy phần đuôi (xem Lỗi 5).
>
> **THÊM MỚI — Kiểm tra nối tiếp (ngăn Lỗi #6):** Dù câu cuối đã có dấu chấm, vẫn phải đọc 3-5 dòng tiếp theo trong OCR. Nếu dòng tiếp theo **bắt đầu bằng chữ thường hoặc là một câu văn hoàn chỉnh không có số footnote mới** → đó là phần nối tiếp của `com()` vừa viết, CHƯA ĐƯỢC ĐÓNG entry lại. Chỉ đóng entry khi dòng tiếp theo là: số footnote mới, `CHAPTER`, running header, hoặc số trang.

### Thứ tự bắt buộc — không được đảo, không được bỏ chương nào

```
[Lặp lại cho TỪNG chương, từ đầu đến cuối sách]

Bước A: grep xác định trang bắt đầu / kết thúc
   ↓
Bước B: grep -n định vị dòng ranh giới → cat đọc RAW từng trang liên quan
         (phải thấy "CHAPTER N+1." trong output thì mới dừng)
   ↓
Bước C: grep đếm số footnote entries
   ↓
Bước D: in danh sách com() sẽ viết (ghi trang nguồn, đánh dấu entry nào tràn trang)
   ↓
Bước E: viết code từng com()
        → sau mỗi cái: sed đọc 5 dòng tiếp theo
        → kiểm tra E-1: câu cuối có dấu chấm?
        → kiểm tra E-2: dòng tiếp theo có phải nối tiếp? (chữ thường, không phải số footnote mới)
        → nếu có nối tiếp: gộp vào com() hiện tại, chưa đóng
   ↓
[Chuyển sang chương tiếp theo → quay lại Bước A]
```

**Nếu bỏ qua bất kỳ bước nào, hoặc bỏ qua bất kỳ chương nào → dừng lại, quay về bước đó trước khi tiếp tục.**

> ⚠️ **Việc đã đọc toàn bộ OCR trước đó KHÔNG miễn cho AI khỏi chạy Protocol này.** Mỗi chương vẫn phải chạy đủ 5 bước độc lập — vì chỉ có grep/cat/sed mới phát hiện được tràn trang và câu bị cụt, không phải trí nhớ từ lần đọc trước. Đặc biệt Bước E-2 là bước duy nhất có thể phát hiện Lỗi #6 (entry nối tiếp sang trang sau dù câu đã có dấu chấm).
...
11. **Chạy PROTOCOL 5 bước cho TỪNG chương, không có ngoại lệ** — kể cả khi đã đọc toàn bộ OCR trước đó: grep xác định trang → grep -n định vị ranh giới → cat đọc RAW → grep đếm entries → in danh sách com() → sed E-1+E-2 kiểm tra câu cuối và nối tiếp. Đọc OCR gộp một lần không thay thế được Protocol này.
12. **Validate PASSED** trước khi `present_files`
13. **CẤMDÙNG** `for i in ...; do cat; done` hoặc `cat *.txt` trước khi bắt đầu viết `com()` — đây là nguyên nhân trực tiếp của Lỗi #6. Nếu cần đọc tổng quan, được phép đọc gộp một lần ở đầu, nhưng **bắt buộc phải chạy lại Protocol 5 bước riêng cho từng chương** trước khi code — không dùng kết quả đọc gộp để viết bất kỳ `com()` nào.
...
| Bỏ sót footnote entry cả chương | Đọc OCR gộp một lần rồi code ngay, không chạy Protocol từng chương | Với MỖI chương: chạy grep/cat/grep đếm/in danh sách/sed theo thứ tự — kể cả khi đã đọc file đó rồi |
| **[Lỗi #6] Com() bị cụt giữa chừng dù câu cuối có dấu chấm** | Entry tràn sang trang sau, phần nối tiếp nằm xen giữa verses của chương tiếp theo trong OCR — dấu chấm kết thúc một *câu*, không phải kết thúc *entry*. Xảy ra khi đọc gộp hoặc không chạy Bước E-2. Ví dụ thực tế: AMOS Ch.III com('12.') thiếu đoạn "Damascus having been conquered by Jeroboam the Second..." nằm ở trang 4 dù com('12.') đã có dấu chấm sau đoạn Henderson. | Sau mỗi com(): dùng `sed -n` đọc 5 dòng tiếp theo trong OCR. Nếu dòng tiếp theo bắt đầu bằng chữ thường và là câu văn (không phải số footnote mới / CHAPTER / cross-ref) → nối tiếp vào com() hiện tại, chưa đóng. Thêm Bước E-2 vào Protocol (xem trên). |
